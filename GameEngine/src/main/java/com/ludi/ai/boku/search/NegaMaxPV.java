...
public class NegaMaxPV implements ISearch {
     ...
     * Constructor
     */
    public NegaMaxPV(IGameAgent agent, IMoveManager movemanager, IHeuristicsManager heuristicsManager, ITranspositionTable ttable) {
            ...
            for (int i = 0; i < legalmoves.size(); ++i) {
                ...
                else // Refutation Checks
                {
                    ...
                    if (gameAgent.isSearchTimeElapsed())
                        break;
                    if((value > alpha) && (value < beta) && (depth-1 != 0))
                    {
    ...
    private Move negamaxSearchMovePV(
        ...
        do{
            ...
            for (int i = 1; i < legalmoves.size(); ++i) {
                ...
                if (gameAgent.isSearchTimeElapsed())
                    break;
                if((score > alpha) && (score < beta) && (depth!=0)) //Re-search is needed now
                {
    ...
    private Move doIterativeDeepening(
        ...
        while (initialdepth <= maxDepth) {
            Move nextBestmove = negamaxSearchMovePV(moveManager, context, initialdepth,currentPly);
            if(gameAgent.isSearchTimeElapsed()) break;
